...
Reminder: number of combinations of k items chosen from a collection of size n: ${n \choose k}$
...
See [clusterProfiler](https://yulab-smu.github.io/clusterProfiler-book/index.html)
...
<!--
Random walks: Weiner process and Brownian bridge
...
We can force the walk to end at 0.
...
The probability of observing the maximum distance observed here may be computed.
...
We will keep the random variable following the Normal distribution. We will make some observations and plot both the observed and expected CDFs.
...
KS test considers that if there is no difference between the two CDFs then that difference should be a
random walk, with both ends fixed at 0.
